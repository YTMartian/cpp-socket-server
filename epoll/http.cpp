    ...
    try {
        if(method == "login") {
            ...
            if(buf_index != -1) {
                ...
                string json_data = "{\"username\":\"" + username_password.first + "\",\"password\":\"" + username_password.second + "\"}";
                Json::Reader reader;
                Json::Value root;
                if (!reader.parse(json_data.c_str(), json_data.c_str() + json_data.size(), root)) {//解析json字符串
